...
- Final Ponytail/Astra Medium review found that replacing an optional converter-dependent test removed its FST escaped-scope assertion. No installed public FST contains an escaped scope, so a narrow optional converter test retains that manual file/bytes check; the installed `fst0087` test still covers escaped variables/aliases in the required gate. The optional check passed with the host GTKWave converter made available only in disposable container scratch; it is not claimed as locked-provider coverage.
