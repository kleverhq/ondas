...
- [x] Milestone 2: reuse public data and supply only missing discriminating artifacts/observations. Provider 4.7.0 is published with the sparse/dense FST and diagnostic FSDB; both new assets were downloaded and checked. The release branch pins 4.7.0, and `fixtures-install`, `just ci` and `just ci-fsdb` pass with the tagged provider. FST 88-entry generate/repeat passes. The new FSDB diagnostic has independently inspected native declarations, an explicit hashed empty-oracle exclusion and a passing focused test. The producer's mandatory complete FSDB gate applies to private delivery, not this public diagnostic; the pre-existing failed public verification is reported separately, not called green. No remote push was performed here.
...
The four milestones are implemented. The complete public FSDB producer verification remains red on unrelated pre-existing inputs; it is not claimed as a successful delivery report or silently skipped.

## Context and Orientation


The analyzed branch is `release/1.0.0-rc` at `4b75d96`. At analysis time `origin/main` was `36659cd`, already containing performance PRs #13–#16; reviewing only `origin/main..HEAD` misses those earlier improvements. At analysis time `fixtures.lock.toml` pinned `kleverhq.ondas-fixtures = 4.6.1`; the release branch now pins the published 4.7.0. Recheck historical anchors before implementing on a later tree.
...
- The user published provider tag `v4.7.0` at `4948a50` with both new assets. The published `fsdb0021` bytes/hash (`a734625e…`) differ from the pre-release diagnostic (`fe3a5c4c…`); a fresh neutral conversion also differs bytewise, while the independent native inventories of all three match exactly (header, two root `top` kinds and variables). Published metadata and focused rejection/metadata tests passed, and the oracle source evidence now names the published hash without claiming byte-identical regeneration. An unmodified provider installer downloaded both published new assets without credentials into a tagged checkout with the 222 unchanged assets preinstalled; `just fixtures-install`, `just ci`, `just ci-fsdb`, MSRV and doctests passed on this released 4.7.0 lock.
- A disposable, detached library commit `185342a` changes only `fixtures.lock.toml` to 4.7.0. Using the exact local `v4.7.0` checkout and its 224 preinstalled, hash-checked artifacts, unmodified `just fixtures-install`, `just ci` (176 FST and 196 VCD oracle cases plus public smoke/package dry run), and `just ci-fsdb` (21 positive public cases, one diagnostic exclusion, focused negative test and vendor smoke) all passed. An initial offline `just ci` failed only because `cargo publish --dry-run` requires a crates.io request; the normal online rerun passed. This was a pre-publication control; the release branch now pins 4.7.0 and passed the same gates against the published tag and assets.
- An isolated pre-release clone at `e2a26f6` (formerly a local `v4.7.0` tag) with all 224 public waveform payloads preinstalled passed the **unmodified** library installer `install(root, lock)` using a disposable lock file set to 4.7.0: tagged HEAD, catalog identity, clean tracked checkout, sizes and hashes all matched, with zero downloads. Both newly prepared gzip assets also round-tripped through the provider's unmodified `install(fixture, file://...)` extraction/checksum path after their payloads were removed from the disposable tagged checkout, and the library installer then revalidated all 224 files. Before publication, a read-only unauthenticated GitHub API enumeration found 222 of 224 exact asset names. The two new assets are now published and were downloaded by the unmodified installer; the release lock was updated only after the remote tag and asset hashes were verified. The benchmark runs above used a different development snapshot with the old lock and the same hashes for timed inputs; the untimed diagnostic FSDB changed bytes at publication.
...
- The oracle producer's `AGENTS.md` requires complete FST/VCD delivery and complete **private** FSDB delivery, but states no mandatory complete public FSDB gate. This public diagnostic has no positive recipe: its hashed empty-oracle exclusion, native inventory and focused library test are the scoped evidence required here. Full public `fsdb.py verify` was attempted and failed on the earlier 0000–0020 baseline; record that result rather than mislabel it a passing corpus report or restore the out-of-scope recipes. No exception to a stated mandatory public gate is inferred.
...
Exact artifact SHA-256 for the matched historical/final benchmark snapshots. The published `fsdb0021` diagnostic is listed separately because its bytes changed before release; no FSDB0021 timing comparison used it:
...
| fsdb0021-conflicting-scope (pre-release diagnostic) | `fe3a5c4cb37cb189d658a07396e3a839f08df82121540768ef5b397da9add5f5` |
| fsdb0021-conflicting-scope (published diagnostic) | `a734625e8efc5d11485136f63e23d94bd3f4812e9c4986592947d4a1c47bf008` |
...
Run focused adapter tests as well; full-pool execution alone does not cover the empty-oracle conflict case. The published 4.7.0 provider and its two new release assets were validated separately after the historical comparisons.
...
The repaired independent FSDB references, targeted correctness regressions, installed-artifact tests, fixture-free/mandatory smoke wiring, and matched benchmark cases are committed in `ondas-lib` as `c8a1ccf` and `0fe9f0b`. The public artifacts were initially committed at `e2a26f6`; the user-published `v4.7.0` now points to `4948a50`, with native-equivalent FSDB diagnostic bytes. The authored sources, bounded FST oracle selection and published diagnostic inventory evidence are committed in the oracle producer as `98d4e8f`, `63034b0`, `9e0d381` and `baec852`. Final source behavior was not altered just to improve a timing result. CI, MSRV, doctests, native vendor and public gates passed using the development snapshot; a disposable worktree passed both CI gates with the pre-release local tag, and the main tree passed `fixtures-install`, `just ci`, `just ci-fsdb`, MSRV and doctests with the user-published 4.7.0 tag and downloaded new assets. A final fixture-free `just check-local` and required-input negative smoke also passed. Historical comparisons distinguish substantial cold and retained-state gains from first-index-build, selection-only and early/short-window regressions; section controls and reversed run order bound those claims. The WIP ledger is local evidence, not a normative performance target.

**Producer baseline limitation:** Full public FSDB producer verification still reports changed metadata on 0000–0009, changed artifact hashes on 0010–0018, and missing 0019/0020 recipes; it was not delivered as a successful corpus report. The producer policy mandates a complete run for **private** FSDB delivery, which this plan explicitly excludes, but does not mandate one for this public diagnostic. Published 0021 has an explicit hashed empty-oracle exclusion, independent native inventory and a passing focused adapter test, not a fabricated positive oracle. Restoring 0019/0020 or claiming complete public-corpus verification is outside this task.
