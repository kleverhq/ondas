...
- [ ] Milestone 2: reuse public data and supply only missing discriminating artifacts/observations. Provider 4.7.0 is published with the sparse/dense FST and diagnostic FSDB; both new assets were downloaded and checked. The release branch now pins 4.7.0, and `fixtures-install`, `just ci` and `just ci-fsdb` pass with the tagged provider. FST 88-entry generate/repeat passes. The older full public FSDB producer gate is still red and needs an explicit scope decision; no remote push was performed here.
...
The remaining public FSDB producer-gate decision is a delivery blocker, not a missing timing run.
...
**Unresolved producer gate:** The older public FSDB producer full-corpus verification still reports changed metadata on 0000–0009, changed artifact hashes on 0010–0018, and missing 0019/0020 recipes. Published diagnostic 0021 has an explicit hashed empty-oracle exclusion, independent native inventory and passing focused adapter test, not a fabricated positive oracle or a green full-corpus report. Its full producer-gate scope needs explicit resolution without making 0019/0020 recipe restoration part of this task.
