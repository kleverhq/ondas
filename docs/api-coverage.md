...
| `Scope::name_was_escaped`, `Variable::name_was_escaped`, `Variable::reader_name` | `escaped_names_preserve_spelling_without_changing_identity` in `tests/vcd_native.rs` checks scopes, aliases, separators and a literal backslash; converter-gated `escaped_declaration_spelling_agrees_with_fst` checks VCD/FST file and byte inputs. `escaped_scalar_suffix_preserves_ambiguous_lookup` in `src/backends/fsdb.rs` checks retained SDK spellings with attached and separated suffixes, and non-FSDB readers return no reader name. |
...
| Per-tick event aggregates | `event_aggregates_preserve_counts_boundaries_and_duplicates`, `event_count_overflow_is_an_error_not_a_partial_tick`; zero/one/multiple counts across samples/scans/traces, gaps/EOF, duplicate slots, candidate union, retained ownership and checked overflow through the memory reader. Native VCD `event_aliases_share_observed_counts_without_multiplication` checks reader aliases. |
...
| Legacy oracle derivation | `tests/support/normalized_oracle.rs` tests net-equal excursions, first unknown establishment, event counts, unchanged serialized evidence, NaN payload/timestamp gaps, chained earlier-window timestamp proofs and typed overlap contradictions without production code. `optional_missing_payload_still_validates_overlapping_evidence` checks preflight before an optional skip. `version_one_validation_does_not_accept_aggregate_value_assertions` preserves the v1 format and checks proven timestamp enforcement; conformance compares indexed records per input slot. |
