...
| `Scope::name_was_escaped`, `Variable::name_was_escaped` | `escaped_names_preserve_spelling_without_changing_identity` in `tests/vcd_native.rs` checks scopes, aliases, separators and a literal backslash. Explicit converter-gated `escaped_declaration_spelling_agrees_with_fst` checks VCD/FST file and byte inputs. |
