...
| Test-only query ownership prototype | `tests/query_contract.rs` holds a real exclusive Selection borrow while generating fixed-size source state; positive tests cover adjacent repeated reads, conditional subsets, owned retention, validation, cancellation and caller errors. Its Rustdoc compile-fail examples reject escaping contexts and samples. This is ownership evidence, not production query execution or backend conformance. |
